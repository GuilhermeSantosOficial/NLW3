...
var icon = L.icon({ //L.icon tem dentro da funcionalidade 1 argumento por enquanto. Um argumento do tipo Objeto. Um objeto tem propriedade e valor. Olhando na documentação desse leaflet (L) a gente já sabe o que esse L.icon esperar ser colocado aqui dentro
    iconUrl: "/images/map-marker.svg", //Esse é o icone, coloquei sua localização. Lembresse de colocar vírgula no final
...
function addMarker({id, name, lat, lng}){
    ...
    }).setContent(`${name} <a href="/orphanage?id=${id}" class="choose-orphanage"> <img src="/images/arrow-white.svg"</a>`) //O L.popup me retorna um popup, (esse popup também é um objeto) e o .setContent("") é a função que vai ser responsável por colocar um conteúdo aqui dentro. E esse conteúdo é do tipo string.)

    //Create and add marker
    L.marker([lat, lng], { icon }).addTo(map) //Aqui a gente coloca outro argumento no L.marker, a dicionando um objeto
